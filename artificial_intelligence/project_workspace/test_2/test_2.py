'''
    ...
    200 images for each category (letter) and 3 categories and 100 training epochs

    Training utilizing cross validation method

    File containing the prepared dataset file is 'test_2/dataset_test_2.arff'
'''
...
#                  store_into_file='test_2/dataset_test_2.arff')

# Load of data from file
loader = LoadData(number_of_images=200,
                  number_of_pixels=784,
                  number_of_categories=3,
                  dataset_file='test_2/dataset_test_2.arff')
...
mlp = MLP(input_images, output_classes, hidden_layer_0=784, output_layer=1, learning_rate=0.9, normalization_factor=10)
...
folds, accuracies = mlp.learn_with_cross_validation(epochs=10, fold_number=5)
...
plt.suptitle('Teste 2.2')
plt.title('Método Cross-Validation - fold #5, utilizando 200 imagens para cada categoria (letra) e 3 categorias\n treinamento durante 10 épocas e taxa de aprendizado = 0.9', fontsize=8)
plt.xlabel('Época')
plt.ylabel('Acurácia %')
plt.ylim(0, 100)
