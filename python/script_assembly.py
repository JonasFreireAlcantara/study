...
# Setup variables
file_source = 'stage_1.fods'    # Fonte
file_destiny = 'stage_1.bin'    # Destino
width_of_matrix = 64        # Mudar isto de acordo com a quantidade de linhas
height_of_matrix = 32       # Mudar isto de acordo com a quantidade de colunas
...
for k in range(int(len(graph) / 6)):
    print("%5s - %5s | %5s | %5s | %5s | %5s | %15s" % (hex(k*24+0x10014000), graph[k*6], graph[k*6 + 1], graph[k*6 + 2], graph[k*6 + 3], graph[k*6 + 4], graph[k*6 + 5]))
print()
